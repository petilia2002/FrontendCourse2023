...
describe('ООП', () => {
    ...
    describe('#Queue', () => {
        ...
        it('методы работают корректно ', () => {
            ...
            // TODO: ваши тесты
            queue.push(...[1, 2, 3]);
            assert.strictEqual(queue.toString(), '1 -2 3 5 1 2 3');
            assert.deepStrictEqual(queue.values, [1, -2, 3, 5, 1, 2, 3]);

            assert.strictEqual(queue.pop(), 1);
            assert.strictEqual(queue.pop(), -2);
            assert.strictEqual(queue.toString(), '3 5 1 2 3');
            assert.deepStrictEqual(queue.values, [3, 5, 1, 2, 3]);
